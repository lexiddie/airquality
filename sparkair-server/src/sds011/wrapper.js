...
class SDS011Wrapper extends EventEmitter {
  ...
  constructor(portPath) {
    ...
    this.query().catch((ex) => {
      console.error('SDS011Wrapper error: ' + ex);
    });
  }
  ...
  _enqueueSetModeCommand(port, state, mode) {
    if (mode !== 'active' && mode !== 'query') throw new Error('Invalid mode');
  ...
  setWorkingPeriod(time) {
    if (time < 0 || time > 30) throw new Error('Invalid argument.');
  ...
  _processCommands() {
    ...
    if (this._retryCount == 0 && cmd !== undefined) cmd.prepare();
